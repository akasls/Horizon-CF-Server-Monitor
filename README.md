...
- **v1.4.0** (2026-09-08): 全面支持 `node_1~4` 自定义监控节点、动态对接 `ping_node_1~4` 与 `loss_node_1~4`；重构延迟与丢包图表体系，智能过滤不存在的节点（包括旧版 `bd` 及未配置节点均不展示），卡片与详情页网络指标自适应。
- **v1.3.0** (2026-09-07): 支持 `/api/config` 最新 `custom_ct_name` / `custom_cu_name` / `custom_cm_name` / `custom_bd_name` 自定义线路/节点名称并全链路适配卡片与图表，无配置时优雅平滑回退，100% 兼容旧版探针后端。
- **v1.2.0** (2026-09-03): 废除线上静默 Demo 覆盖、增加私有站点 401 登录与 403 人机验证引导、细化数据库升级 409 与网络重试状态、空服务器状态解耦、强化指标计算防御性。
